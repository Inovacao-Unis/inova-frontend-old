...
const PainelAdmin = ({ trail }) => {
  ...
  const handleModal = (team, response) => {
    setSelect({ team, response });
    onOpen();
  };
  ...
  const handlePoints = async () => {
    ...
    await api
      .post('points', {
        value: points,
        feedback,
        responseId: select.response._id,
        leaderId: leader,
        trailId: select.response.trailId,
        teamId: select.response.teamId,
      })
      .then(() => {
        setPoints(null);
        setFeedback(null);
              ...
              <ModalContent>
                <ModalHeader>
                  <Text fontSize=".8rem" color="gray.500">
                    Etapa: {select?.response?.stage}
                  </Text>
                  <Text>{select?.team?.name}</Text>
                </ModalHeader>
                <ModalCloseButton />
                <ModalBody>
                  <Flex direction="column">
                    <Text fontWeight="bold">Resposta:</Text>
                    <Link href={select?.response?.response} isExternal>
                      <Text color="highlight">
                        {select?.response?.response}
                      </Text>
                    </Link>
                  </Flex>
                  {select?.response?.points ? (
                    <Flex direction="column">
                      <Text pt="10px" fontWeight="bold">
                        Pontos:
                      </Text>
                      <Text>{select?.response?.points?.value}</Text>
                      <Text pt="10px" fontWeight="bold">
                        Feedback:
                      </Text>
                      <Text>{select?.response?.points?.feedback}</Text>
                    </Flex>
                  ) : (
                    <Flex direction="column">
